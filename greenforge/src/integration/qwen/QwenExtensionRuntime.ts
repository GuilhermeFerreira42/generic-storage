...
class InternalMockLLMProvider implements LLMProvider {
  async generate(prompt: string): Promise<string> {
    if (prompt.includes('Classifique a intenção') || prompt.includes('classify')) {
      // Fase 24: detecção ampliada de conversa casual para evitar falsos positivos DEVELOPMENT_TASK.
      // O mock agora identifica saudações, agradecimentos e perguntas de conversa em pt/en.
