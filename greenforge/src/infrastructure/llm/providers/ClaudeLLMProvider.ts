...
/**
 * ClaudeLLMProvider — safe stub for Anthropic Claude API.
 *
  ...
  async generate(prompt: string): Promise<string> {
    ...
    if (!this.transport) {
      throw new LLMProviderError(
        'NO_TRANSPORT',
        `ClaudeLLMProvider: No transport configured. Cannot make real API calls without an injected transport. Provider: claude, Model: ${this.config.model ?? 'default'}`,
        'claude',
        false,
      );
    }
    ...
    if (!this.config.apiKeyEnv) {
      throw new LLMProviderError(
        'NO_API_KEY_CONFIG',
        'ClaudeLLMProvider: No apiKeyEnv configured. Cannot authenticate without an API key environment variable name.',
        'claude',
        false,
      );
    }
    ...
    if (!apiKey) {
      throw new LLMProviderError(
        'NO_API_KEY',
        `ClaudeLLMProvider: Environment variable ${this.config.apiKeyEnv} is not set. Cannot authenticate.`,
        'claude',
        false,
      );
    }
    ...
    if (response.status !== 200) {
      throw new LLMProviderError(
        'API_ERROR',
        `ClaudeLLMProvider: API returned status ${response.status}`,
        'claude',
        response.status >= 500,
      );
    }
