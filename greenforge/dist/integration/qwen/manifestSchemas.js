...
export const REQUIRED_SKILL_COMMANDS = ['start', 'status', 'list', 'approve', 'abort'];
...
export const QwenExtensionManifestSchema = z
    ...
    mcpServers: z.record(z.string().min(1), McpServerSchema).refine((servers) => Object.keys(servers).length > 0, 'at least one mcpServer must be declared'),
    skills: LocalPathSchema,
    contextFileName: LocalPathSchema.optional(),
    hooks: LocalPathSchema.optional(),
})
    .strict();
export function validateQwenExtensionManifest(input) {
...
export function validateSkillManifest(markdown) {
...
export function skillListsRequiredCommands(markdownBody) {
...
export function validateQwenSettings(input) {
    return QwenSettingsSchema.parse(input);
}
export function settingsProtectsSensitiveTools(settings, tools) {
...
export function collectManifestLocalPaths(manifest) {
    return [manifest.skills, manifest.hooks, manifest.contextFileName].filter((value) => Boolean(value));
}
