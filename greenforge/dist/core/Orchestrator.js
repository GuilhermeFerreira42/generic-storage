export class Orchestrator {
    ...
    async trigger(taskId, event) {
        ...
        let nextStatus = null;
        const metadata = { from: currentStatus, event };
        switch (currentStatus) {
            ...
            case 'PLANNING':
                ...
                if (event === 'START_BUILD') {
                    ...
                    if (!isApproved)
                        throw new Error('Invalid transition: Plan not approved');
                    const graph = this.repository.getSubtasksGraph(taskId);
                    if (!graph || graph.length === 0)
                        throw new Error('Invalid transition: No subtasks');
            ...
            case 'BUILDING_PARALLEL':
                if (event === 'BUILD_DONE') {
                    if (currentStatus === 'BUILDING_PARALLEL') {
                        ...
                        if (!allDone)
                            throw new Error('Invalid transition: Not all subtasks are DONE');
                        nextStatus = 'JOINING';
                    }
                    else {
            ...
            case 'JOINING':
                if (event === 'BUILD_DONE') {
                    ...
                    if (!allArtifactsPresent)
                        throw new Error('Invalid transition: Missing artifactOutput');
                    nextStatus = 'REVIEWING';
                }
                break;
            case 'REVIEWING':
                ...
                if (event === 'REVIEW_VIOLATIONS')
                    nextStatus = 'BUILDING';
                break;
            case 'VERIFYING':
                if (event === 'VERIFY_SUCCESS')
