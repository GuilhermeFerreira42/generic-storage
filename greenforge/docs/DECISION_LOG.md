...
F25 | VAL | Comportamento real do Qwen CLI ainda não é determinístico | O teste externo confirmou discovery do servidor MCP e do hook, mas o modelo final preferiu tools nativas (`write_file`, `agent`) e não iniciou o fluxo GreenForge/Git/worktree de forma confiável; decisão: manter as correções locais validadas e seguir com hardening do handoff via prompt/hook/command path antes de considerar a integração pronta para produção | `src/integration/qwen/QwenHookHandler.ts`, `src/integration/qwen/HookCommandAdapter.ts`, `src/integration/qwen/QwenCommandHandler.ts`, `src/integration/qwen/McpGreenForgeServer.ts`
